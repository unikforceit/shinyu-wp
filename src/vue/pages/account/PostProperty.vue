...
.post-property
  validation-observer(ref='postPropertyForm')
    form(@submit.prevent='onSubmit')
      ...
      b-field.mb-4(v-if='form.transaction', grouped, expanded)
        validation-provider.field.is-expanded.field-group(
          ...
          b-field(
            ...
            .control
              span.button.is-static {{ i18n.baht }}
      ...
      b-field(v-if='form.transaction')
        b-checkbox(v-model='form.include_commission', size='is-small') {{ i18n.post_property.include_commission }}
          ...
          section.section
            .content.has-text-centered
              p
                ...
                p.has-text-gray(v-html='i18n.post_property.drag_drop_or_click')
      ...
      ul.columns.is-multiline.is-1.is-variable.image-preview
        li.column.is-2(v-for='(image, index) in form.images', :key='index')
          img(:src='image.url')
          button.delete.is-small(type='button', @click='deleteDropFile(index)')
...
<script>
import { axiosNonce, axios } from '../../config.js'
...
.has-addons .control.is-clearfix {
  width: 100%;
}
