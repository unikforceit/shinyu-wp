...
class Mimo_Form_API {
  ...
  public function rest_api_init() {
    ...
    register_rest_route( 'sy', '/form/request-viewing', array(
      'methods'  => 'POST',
      'callback' => array( $this, 'request_view' ),
    ...
    register_rest_route( 'sy', '/form/your-need/field', array(
      'methods'  => 'get',
      'callback' => array( $this, 'your_need_field' ),
      'args'     => array( 'lang'),
    ) );
...
อีเมล์นี้ถูกส่งจาก ' . site_url();
  ...
  public function your_need($data){
...
อีเมล์นี้ถูกส่งจาก ' . site_url();
  ...
  public function your_need_field($data){
  ...
  public function request_view($data){
    ...
    if ( $errors === false ) {
      ...
      update_post_meta( $post_id, 'request_date', date_i18n( 'Y-m-d H:i', strtotime( str_replace( '/', '-', $date . ' ' . $time ) ) ) );
      ...
      $emai_message = $email_class->header( $email_subject );
      $emai_message .= $line_message =  '
<strong>จาก : </strong>' . get_post_type($form_id). ' ' .get_the_title($form_id) . '<br/>
...
อีเมล์นี้ถูกส่งจาก ' . site_url();
      ...
      line_notify( PHP_EOL . wp_strip_all_tags( $line_message ), get_field( 'request_line_token', 'option' ) );
  ...
  public function upload_image($data){
    ...
    if ( is_wp_error( $attachment_id ) ) {
      ...
      return array(
        'id'   => $attachment_id,
        'url'  => $url,
        'name' => get_the_title($attachment_id) . '.' . $filetype['ext'],
      );
